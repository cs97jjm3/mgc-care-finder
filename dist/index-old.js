...
function formatDataAge(isoDate) {
    if (!isoDate)
...
async function cqcRequest(endpoint, params = {}) {
...
async function getLocationDetails(locationId) {
    return await cqcRequest(`/locations/${locationId}`);
}
async function enrichLocations(locationIds, maxConcurrent = 10) {
    const results = [];
    for (let i = 0; i < locationIds.length; i += maxConcurrent) {
        const batch = locationIds.slice(i, i + maxConcurrent);
        const batchResults = await Promise.all(batch.map(id => getLocationDetails(id).catch(() => null)));
...
// Tool Handlers
// =============================================================================
function formatCQCLocation(loc) {
    const lines = [];
    ...
    if (loc.registrationStatus)
        lines.push(`Status: ${loc.registrationStatus}`);
    const addressParts = [];
    if (loc.postalAddressLine1)
        addressParts.push(loc.postalAddressLine1);
    if (loc.postalAddressTownCity)
        addressParts.push(loc.postalAddressTownCity);
    if (loc.postalCode)
        addressParts.push(loc.postalCode);
    if (addressParts.length > 0)
        lines.push(`Address: ${addressParts.join(", ")}`);
    if (loc.localAuthority)
        lines.push(`Local Authority: ${loc.localAuthority}`);
    if (loc.region)
        lines.push(`Region: ${loc.region}`);
    if (loc.mainPhoneNumber)
        lines.push(`Phone: ${loc.mainPhoneNumber}`);
    if (loc.currentRatings?.overall) {
        lines.push(`\n**CQC Ratings:**`);
        lines.push(`  Overall: ${loc.currentRatings.overall.rating}`);
        if (loc.currentRatings.safe)
            lines.push(`  Safe: ${loc.currentRatings.safe.rating}`);
        if (loc.currentRatings.effective)
            lines.push(`  Effective: ${loc.currentRatings.effective.rating}`);
        if (loc.currentRatings.caring)
            lines.push(`  Caring: ${loc.currentRatings.caring.rating}`);
        if (loc.currentRatings.responsive)
            lines.push(`  Responsive: ${loc.currentRatings.responsive.rating}`);
        if (loc.currentRatings.wellLed)
            lines.push(`  Well-Led: ${loc.currentRatings.wellLed.rating}`);
    }
    if (loc.lastInspection?.date)
        lines.push(`\nLast Inspection: ${loc.lastInspection.date}`);
    if (loc.gacServiceTypes?.length)
        lines.push(`Service Types: ${loc.gacServiceTypes.map(s => s.name).join(", ")}`);
    if (loc.specialisms?.length)
        lines.push(`Specialisms: ${loc.specialisms.map(s => s.name).join(", ")}`);
    return lines.join("\n");
}
async function handleSearchCareProviders(args) {
    const params = {
        page: args.page || 1,
        perPage: Math.min(args.perPage || 20, 50)
    };
    if (args.careHome)
        params.careHome = String(args.careHome);
    if (args.region)
        params.region = String(args.region);
    if (args.localAuthority)
        params.localAuthority = String(args.localAuthority);
    if (args.rating)
        params.overallRating = String(args.rating);
    const data = await cqcRequest("/locations", params);
...
async function handleSearchByPostcode(args) {
    ...
    const maxResults = args.perPage || 20;
...
async function handleGetProviderDetails(args) {
...
async function handleGetRatingsSummary(args) {
    ...
    result += `Data source: CQC API (Live) ✅\n`;
    if (args.localAuthority)
        result += `Local Authority: ${args.localAuthority}\n`;
    if (args.region)
...
async function handleLookupPostcode(args) {
    ...
    if (data.admin_county)
        result += `County: ${data.admin_county}\n`;
...
function handleSearchIreland(args) {
    ...
    limited.forEach(p => {
        ...
        if (p.personInCharge)
            result += `Person in Charge: ${p.personInCharge}\n`;
...
function handleSearchNorthernIreland(args) {
    ...
    limited.forEach(p => {
        ...
        if (p.address)
            result += `Address: ${p.address}\n`;
        if (p.postcode)
            result += `Postcode: ${p.postcode}\n`;
        if (p.phone)
            result += `Phone: ${p.phone}\n`;
        result += `Provider: ${p.providerName}\n`;
        result += `District: ${p.localGovernmentDistrict}\n`;
        if (p.lastInspectionDate)
            result += `Last Inspection: ${p.lastInspectionDate}\n`;
        result += `\n---\n\n`;
    });
    if (filtered.length > maxResults) {
        result += `*Showing ${maxResults} of ${filtered.length} results*`;
...
function handleSearchScotland(args) {
    const serviceType = args.serviceType ? String(args.serviceType).toLowerCase() : "";
...
function handleCheckDataFreshness() {
...
// Server Setup
// =============================================================================
const server = new Server({ name: "care-provider-finder", version: "2.0.0" }, { capabilities: { tools: {} } });
