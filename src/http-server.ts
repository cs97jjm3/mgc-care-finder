...
app.get('/api/search/outstanding', async (req, res) => {
  try {
    ...
    if (careHome) url += `&careHome=${careHome}`;
    if (region) url += `&region=${encodeURIComponent(region as string)}`;
    url += `&perPage=100`;
    ...
    const filtered = detailedLocations
      .filter((loc): loc is NonNullable<typeof loc> => 
        loc !== null && loc.registrationStatus === 'Registered'
      );
...
app.get('/api/search/at-risk', async (req, res) => {
  try {
    ...
    for (const r of ratings) {
      let url = `${CQC_API_BASE}/locations?overallRating=${encodeURIComponent(r as string)}`;
      if (careHome) url += `&careHome=${careHome}`;
      if (region) url += `&region=${encodeURIComponent(region as string)}`;
      url += `&perPage=100`;
    ...
    const filtered = detailedLocations.filter((loc): loc is NonNullable<typeof loc> => 
      loc !== null && loc.registrationStatus === 'Registered'
    );
    ...
    res.json({
      ...
      locations: filtered.sort((a, b) => {
        if (a.rating === 'Inadequate' && b.rating !== 'Inadequate') return -1;
        if (a.rating !== 'Inadequate' && b.rating === 'Inadequate') return 1;
        return 0;
      })
    });
...
app.get('/api/search/recent-inspections', async (req, res) => {
  try {
    ...
    const recentlyInspected = allLocations.filter(loc => {
      if (!loc.lastInspection?.date) return false;
...
app.get('/api/search/large-homes', async (req, res) => {
  try {
    ...
    const detailedLocations = await Promise.all(
      allLocations.slice(0, 200).map(async (loc) => {
        try {
          const detailRes = await fetch(`${CQC_API_BASE}/locations/${loc.locationId}`, {
            headers: { 'Ocp-Apim-Subscription-Key': CQC_SUBSCRIPTION_KEY }
          });
  ...
  try {
    ...
    if (providerName) {
      allLocations = allLocations.filter(loc => 
        loc.organisationName?.toLowerCase().includes((providerName as string).toLowerCase())
      );
    }
    const detailedLocations = await Promise.all(
      allLocations.map(async (loc) => {
    ...
    const filtered = detailedLocations.filter((loc): loc is NonNullable<typeof loc> => loc !== null);
    const stats = {
    ...
    const detailedLocations = await Promise.all(
      allLocations.slice(0, 200).map(async (loc) => {
        try {
          ...
          const details = await detailRes.json();
          const hasService = details.specialisms?.some((s: any) => 
            s.name?.toLowerCase().includes((serviceType as string).toLowerCase())
          ) || details.gacServiceTypes?.some((s: any) => 
            s.name?.toLowerCase().includes((serviceType as string).toLowerCase())
          );
          if (hasService) {
            return {
              ...
              beds: details.numberOfBeds,
              rating: details.currentRatings?.overall?.rating,
              serviceTypes: details.gacServiceTypes?.map((s: any) => s.name).join(', '),
              specialisms: details.specialisms?.map((s: any) => s.name).join(', '),
              phone: details.mainPhoneNumber,
              registrationStatus: details.registrationStatus,
              cqcReportUrl: `https://www.cqc.org.uk/location/${loc.locationId}`
            };
      ...
      regions.map(async (region) => {
        try {
          const response = await fetch(
            `${CQC_API_BASE}/locations?region=${encodeURIComponent(region)}&careHome=${careHome}&perPage=1`,
            { headers: { 'Ocp-Apim-Subscription-Key': CQC_SUBSCRIPTION_KEY } }
          );
          const data = await response.json();
          const ratings = await Promise.all(['Outstanding', 'Good', 'Requires improvement', 'Inadequate'].map(async (rating) => {
            const ratingRes = await fetch(
              `${CQC_API_BASE}/locations?region=${encodeURIComponent(region)}&careHome=${careHome}&overallRating=${rating}&perPage=1`,
              { headers: { 'Ocp-Apim-Subscription-Key': CQC_SUBSCRIPTION_KEY } }
            );
...
app.get('/api/compare/authorities', async (req, res) => {
  try {
    ...
    const authorityList = (authorities as string).split(',').map(a => a.trim());
    const authorityStats = await Promise.all(
      authorityList.map(async (authority) => {
        try {
          const response = await fetch(
            `${CQC_API_BASE}/locations?localAuthority=${encodeURIComponent(authority)}&careHome=${careHome}&perPage=1`,
            { headers: { 'Ocp-Apim-Subscription-Key': CQC_SUBSCRIPTION_KEY } }
          );
          const data = await response.json();
          const ratings = await Promise.all(['Outstanding', 'Good', 'Requires improvement', 'Inadequate'].map(async (rating) => {
            const ratingRes = await fetch(
              `${CQC_API_BASE}/locations?localAuthority=${encodeURIComponent(authority)}&careHome=${careHome}&overallRating=${rating}&perPage=1`,
              { headers: { 'Ocp-Apim-Subscription-Key': CQC_SUBSCRIPTION_KEY } }
            );
            const ratingData = await ratingRes.json();
            return { rating, count: ratingData.total || 0 };
